...
namespace TKPhysics.AutomaticDynamicBone.Runtime
{
    public class ADBChainProcessor: ADBRuntimePoint, IADBPhysicMonoComponent
    {
        ...
        #region Gizmo
        public void DrawGizmos(ColliderCollisionType colliderCollisionType)
        {
        ...
        private void DrawConstraint(List<ADBRuntimeConstraint> constraints, ColliderCollisionType colliderCollisionType)
        {
            if (constraints == null) return;
            for (int i = 0; i < constraints.Count; i++)
            {
                constraints[i].OnDrawGizmos(colliderCollisionType == ColliderCollisionType.Constraint || colliderCollisionType == ColliderCollisionType.Both);
            }
        }
