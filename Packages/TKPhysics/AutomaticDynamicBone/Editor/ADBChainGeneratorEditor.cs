...
using System.Linq;
using TKPhysics.AutomaticDynamicBone.Runtime.Tool;
using UnityEditor;
...
namespace TKPhysics.AutomaticDynamicBone.Runtime.Editor
{
    ...
    public class ADBChainGeneratorEditor : UnityEditor.Editor
    {
        ADBChainGenerateTool controller;
        private bool isFoldout;
        private bool[] isFoldouts;
        private Dictionary<Object, UnityEditor.Editor> containerEditors;
        ...
        void ShowContainerGUI<T>(T container) where T : MonoBehaviour
        {
            ...
            if (containerEditors == null) containerEditors = new Dictionary<Object, UnityEditor.Editor>();
            if (containerEditors.TryGetValue(container, out UnityEditor.Editor editor))
            {
                editor.OnInspectorGUI();
            }
            else
            {
                containerEditors.Add(container, UnityEditor.Editor.CreateEditor(container));
            }
        }
    }
